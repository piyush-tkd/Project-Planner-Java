...
/**
 * Builds a Spring Security {@link ClientRegistrationRepository} backed by the
 * {@code sso_config} database row.  Because this is a {@code @Bean} it is
 * resolved once at context start-up; the SSO redirect URI and provider are
 * used as-configured at that point.  A future enhancement could make this
 * lazily-refreshing, but for now a server restart is required to pick up
 * provider changes (client-secret updates are handled separately).
 *
 * <p>If SSO is disabled or the {@code sso_config} row is incomplete the bean
 * returns an empty repository, which causes Spring Security to skip OAuth2
 * login silently.
 */
@Slf4j
@Configuration
...
public class SsoClientRegistrationConfig {
    ...
    public ClientRegistrationRepository clientRegistrationRepository() {
        ...
        ClientRegistration registration = buildRegistration(cfg);
        if (registration == null) {
            return registrationId -> null;
        }

        log.info("SSO: registering OAuth2 client '{}' with provider {}",
    ...
    private ClientRegistration buildRegistration(SsoConfig cfg) {
        ...
        SsoProvider provider = cfg.getProvider() != null ? cfg.getProvider() : SsoProvider.GOOGLE;
        ...
        switch (provider) {
            ...
            case OKTA, CUSTOM -> {
                ...
                builder
                    ...
                    .userNameAttributeName("email")
                    .clientName(provider == SsoProvider.OKTA ? "Okta" : "SSO");
            }
        }
