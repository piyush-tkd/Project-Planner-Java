...
import { DEEP_BLUE_HEX, DEEP_BLUE_TINTS, FONT_FAMILY } from '../../../../brandTokens';
...
export function CountdownDisplay({ targetDate, label }: { targetDate: string; label?: string }) {
  ...
  const digitBox = (val: number, unit: string) => (
    <div style={{ textAlign: 'center' }}>
      <div style={{
        background: isPast ? '#EF4444' : `linear-gradient(135deg, ${DEEP_BLUE_HEX} 0%, ${DEEP_BLUE_TINTS[80]} 100%)`,
        borderRadius: 8, padding: '8px 12px', minWidth: 56,
        boxShadow: '0 2px 8px rgba(0,0,0,0.15)',
      }}>
        <Text size="2rem" fw={800} style={{ color: '#fff', fontFamily: FONT_FAMILY, lineHeight: 1 }}>
