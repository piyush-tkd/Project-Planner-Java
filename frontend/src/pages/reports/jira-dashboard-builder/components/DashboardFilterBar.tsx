import { Button, MultiSelect, Menu, ActionIcon, Text, Loader, Paper } from '@mantine/core';
import {
  IconCalendarStats, IconTrash, IconSettings, IconPlus, IconTemplate,
  IconDeviceFloppy, } from '@tabler/icons-react';
import { AQUA_HEX, BORDER_STRONG, FONT_FAMILY } from '../../../../brandTokens';

interface DashboardFilterBarProps {
  dark: boolean;
  datePreset: 'week' | 'month' | 'quarter' | 'year' | 'last30' | 'last90' | 'custom';
...
export function DashboardFilterBar({
  ...
  return (
        ...
        <Menu shadow="md" width={240}>
          ...
          <Menu.Dropdown>
            ...
            {[
              ...
              <Menu.Item
                key={v}
                onClick={() => onDatePreset(v)}
                style={{
                  fontWeight: datePreset === v ? 700 : 400,
                  color: datePreset === v ? AQUA_HEX : undefined,
                }}
              >
                {l}
              </Menu.Item>
            ...
            <Menu.Divider />
            <Menu.Item
              onClick={() => onDatePreset('last30')}
              style={{
                fontWeight: datePreset === 'last30' ? 700 : 400,
                color: datePreset === 'last30' ? AQUA_HEX : undefined,
              }}
            >
              Last 30 Days
            </Menu.Item>
            <Menu.Item
              onClick={() => onDatePreset('last90')}
              style={{
                fontWeight: datePreset === 'last90' ? 700 : 400,
                color: datePreset === 'last90' ? AQUA_HEX : undefined,
              }}
            >
              Last 90 Days
            </Menu.Item>
        ...
        <>
          <div
            style={{
              width: 1,
              height: 20,
              background: dark ? 'rgba(255,255,255,0.12)' : BORDER_STRONG,
            }}
          />
          <Button
            size="xs"
    ...
    >
          ...
          <Menu.Dropdown>
            <Menu.Item onClick={onNewDashboard}>New Dashboard</Menu.Item>
            <Menu.Item onClick={onLoadDashboard}>My Dashboards</Menu.Item>
            {onCloneDashboard && <Menu.Item onClick={onCloneDashboard}>Clone This Dashboard</Menu.Item>}
            <Menu.Divider />
            <Menu.Item onClick={onExportDashboard}>Export as CSV</Menu.Item>
            <Menu.Item onClick={onRefreshData}>Refresh Data</Menu.Item>
            <Menu.Item onClick={() => onSync(false)} disabled={syncRunning}>
