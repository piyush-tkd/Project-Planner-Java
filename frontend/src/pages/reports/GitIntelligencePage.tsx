...
import {
 ...
 Paper, Box, Progress, ScrollArea, Alert, ThemeIcon,
 Tabs
} from '@mantine/core';
import {
...
import {
 ...
 IconClock, IconAlertCircle, IconSettings
} from '@tabler/icons-react';
import { useNavigate } from 'react-router-dom';
import {
 useAdoStatus, useAdoSummary, useAdoPrs, useAdoCommits, useAdoBranches
} from '../../api/azureDevOps';
import { AQUA, COLOR_AMBER, COLOR_BLUE, COLOR_ERROR, COLOR_ERROR_STRONG, COLOR_SUCCESS, COLOR_TEAL, COLOR_VIOLET_ALT, COLOR_WARNING, DEEP_BLUE, TEXT_DIM, TEXT_GRAY, TEXT_SUBTLE} from '../../brandTokens';
...
function NotConfigured({ onConfigure }: { onConfigure: () => void }) {
 ...
 <Title order={3} mb={6} style={{ color: DEEP_BLUE}}>
...
function KpiCard({
 ...
 style={{ color: TEXT_SUBTLE, letterSpacing: '0.6px'}}>
 {label}
 </Text>
 <ThemeIcon variant="light" color={color} size={32} radius="md">{icon}</ThemeIcon>
 </Group>
 <Text fw={800} size="xl" style={{ color: DEEP_BLUE}}>{value}</Text>
...
export default function GitIntelligencePage() {
 ...
 <Box>
 <Title order={2} style={{ color: DEEP_BLUE, fontWeight: 800 }}>
 Git Intelligence
 </Title>
