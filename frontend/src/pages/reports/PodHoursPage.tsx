...
function DeltaBadge({ current, previous }: { current: number; previous: number }) {
  ...
  return (
    <Group gap={3} mt={3}>
      ...
      <Text size="xs" style={{ color, fontFamily: FONT_FAMILY }}>{label} vs prior period</Text>
    </Group>
  );
}
...
function PeriodSelector({ year, setYear, period, setPeriod, periodIndex, setPeriodIndex, dark }: PeriodSelectorProps) {
  ...
  const pillStyle = (active: boolean): React.CSSProperties => ({
    ...
    fontFamily: FONT_FAMILY, cursor: 'pointer', border: 'none',
...
function StatCard({ label, value, sub, icon, color, dark, prevValue }: {
  ...
  return (
    <Box style={{
      ...
      <Text fw={700} size="xl" style={{ color: dark ? '#fff' : DEEP_BLUE, fontFamily: FONT_FAMILY }}>
        {value}
      </Text>
      ...
      <Stack gap="sm">
        <Group gap={24}>
          <Box>
            ...
            <Text fw={700} size="lg" style={{ color: BUFFER_COLOR, fontFamily: FONT_FAMILY }}>{fmtHours(totalBufferHours)}</Text>
          </Box>
          <Box>
            <Text size="xs" c="dimmed">% of POD Total</Text>
            <Text fw={700} size="lg" style={{ color: BUFFER_COLOR, fontFamily: FONT_FAMILY }}>{bufferPct}%</Text>
          </Box>
          <Box>
            <Text size="xs" c="dimmed">POD Total</Text>
            <Text fw={700} size="lg" style={{ color: dark ? '#fff' : DEEP_BLUE, fontFamily: FONT_FAMILY }}>{fmtHours(pod.totalHours)}</Text>
          </Box>
        </Group>
        ...
        <ScrollArea mah={360}>
          <Table withColumnBorders style={{ fontFamily: FONT_FAMILY }}>
            <Table.Thead>
              <Table.Tr>
                {['Resource', 'Home POD', 'Role', 'Hours', 'Issues'].map(h => (
                  <Table.Th key={h} style={{
                    background: dark ? 'rgba(255,255,255,0.04)' : 'rgba(12,35,64,0.04)',
                    color: dark ? '#fff' : DEEP_BLUE, fontFamily: FONT_FAMILY,
                    fontWeight: 700, fontSize: 12, padding: '8px 10px',
                  }}>{h}</Table.Th>
                ))}
              </Table.Tr>
            </Table.Thead>
              ...
              {buffers.map((b, i) => (
                <Table.Tr key={b.name} style={{ background: i % 2 === 1 ? (dark ? 'rgba(255,255,255,0.02)' : 'rgba(12,35,64,0.015)') : 'transparent' }}>
                  <Table.Td style={{ padding: '8px 10px' }}>
                    <Text size="sm" fw={600} style={{ color: dark ? '#fff' : DEEP_BLUE, fontFamily: FONT_FAMILY }}>{b.name}</Text>
                  </Table.Td>
                  <Table.Td style={{ padding: '8px 10px' }}>
                  ...
                  <Table.Td style={{ padding: '8px 10px', textAlign: 'right' }}>
                    <Text size="sm" fw={700} style={{ color: BUFFER_COLOR, fontFamily: FONT_FAMILY }}>{fmtHours(b.hours)}</Text>
                  </Table.Td>
    ...
    <Box style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 20 }}>
      {podSummaries.map(pod => (
        <Box key={pod.podId} style={{
          ...
          <Text size="xs" fw={700} style={{ color: AQUA, fontFamily: FONT_FAMILY, marginBottom: 4 }}>
            {pod.podName}
          </Text>
          <Text fw={700} size="lg" style={{ color: dark ? '#fff' : DEEP_BLUE, fontFamily: FONT_FAMILY }}>
...
function MatrixTab({ resources, podNames, dark, search }: {
  ...
  const thStyle = (col: string): React.CSSProperties => ({
    ...
    fontFamily: FONT_FAMILY, fontWeight: sortCol === col ? 700 : 600,
  ...
  return (
    <ScrollArea style={{ width: '100%' }}>
      <Table style={{ borderCollapse: 'collapse', minWidth: 600, fontFamily: FONT_FAMILY }} withColumnBorders>
        <Table.Thead>
          <Table.Tr>
            {/* Resource column */}
            <Table.Th
            ...
            return (
              <Table.Tr key={res.authorName} style={{ background: idx % 2 === 1 ? rowAlt : 'transparent' }}>
                ...
                <Table.Td style={{
                  ...
                  <Text size="sm" fw={600} style={{ color: dark ? '#fff' : DEEP_BLUE, fontFamily: FONT_FAMILY }}>
                    {res.authorName}
                  </Text>
                  <Group gap={4} mt={2}>
                    {res.role && (
                ...
                <Table.Td style={{ padding: '8px', textAlign: 'right' }}>
                  <Text size="sm" fw={700} style={{ color: AQUA, fontFamily: FONT_FAMILY }}>
        ...
        <Table.Thead>
          <Table.Tr>
            {['POD', 'Resource', 'Role', 'Home POD', 'Hours', 'Issues', 'Type'].map(h => (
              <Table.Th key={h} style={{
                background: headerBg, color: dark ? '#fff' : DEEP_BLUE,
                fontFamily: FONT_FAMILY, fontWeight: 700, fontSize: 12, padding: '10px 12px',
              }}>{h}</Table.Th>
            ))}
          </Table.Tr>
        </Table.Thead>
...
export default function PodHoursPage() {
  ...
  return (
    <Box p="lg" style={{ fontFamily: FONT_FAMILY }}>
      ...
      {/* Period label */}
      {data && (
        <Text size="sm" fw={600} mb="md" style={{ color: AQUA, fontFamily: FONT_FAMILY }}>
          {data.periodLabel}
        </Text>
      )}
