...
import { AQUA_HEX, AQUA, COLOR_BLUE, COLOR_BLUE_STRONG, COLOR_EMERALD, COLOR_ERROR_DARK, COLOR_ERROR_STRONG, COLOR_TEAL, COLOR_VIOLET, COLOR_VIOLET_ALT, COLOR_WARNING, DEEP_BLUE, SLATE_700, TEXT_GRAY, TEXT_SUBTLE} from '../../brandTokens';
import { useResources } from '../../api/resources';
import { useCostRates } from '../../api/costRates';
import { useProjects } from '../../api/projects';
import { useWorklogReport } from '../../api/jira';
...
export default function FinancialIntelligencePage() {
 ...
 const { data: _metrics, isLoading: mLoading } = useProductivityMetrics(6);
 ...
 <Text c="dimmed" size="sm" mt={2} style={{ }}>
 ...
 size="sm"
 label="Month"
 styles={{ label: {fontWeight: 600, fontSize: 11 } }}
 />
 </Group>
 ...
 <Text size="xs" tt="uppercase" fw={700} style={{ color: TEXT_SUBTLE, letterSpacing: '0.6px'}}>
 {k.label}
 </Text>
 <ThemeIcon variant="light" color={k.color} size={32} radius="md">{k.icon}</ThemeIcon>
 </Group>
 <Text fw={800} size="xl" style={{ color: DEEP_BLUE}}>{k.value}</Text>
 <Text size="xs" c="dimmed" mt={2} style={{ }}>{k.sub}</Text>
 </Card>
 ))}
 </SimpleGrid>
 ...
 <Text size="xs" style={{ color: DEEP_BLUE}}>{it.type}</Text>
 </Group>
 <Group gap="md">
 ...
 <Group gap={8}>
 <Text size="xs" fw={600} style={{ color: DEEP_BLUE, width: 150}}>{pod.pod}</Text>
 ...
 <Text fw={700} size="sm" style={{ color: DEEP_BLUE}}>Planned vs Unplanned Work Split</Text>
 {unplannedPct > 30 && (
 ...
 <Group justify="space-between" mb="sm">
 <Text fw={700} size="sm" style={{ color: DEEP_BLUE}}>{pod.pod}</Text>
 ...
 <Text size="xs" style={{ color: DEEP_BLUE}}>{d.name}</Text>
 </Group>
 ...
 <Table.Tr>
 <Table.Th style={{fontWeight: 600, fontSize: 11 }}>Project</Table.Th>
 <Table.Th style={{fontWeight: 600, fontSize: 11 }} ta="right">Estimated</Table.Th>
 <Table.Th style={{fontWeight: 600, fontSize: 11 }} ta="right">Actual</Table.Th>
 <Table.Th style={{fontWeight: 600, fontSize: 11 }} ta="right">% Used</Table.Th>
 <Table.Th style={{fontWeight: 600, fontSize: 11 }} ta="right">Remaining</Table.Th>
 </Table.Tr>
 </Table.Thead>
 <Table.Tbody>
 {budgetAtRisk.map((p) => {
 ...
 <Table.Td style={{ }}>{p.name}</Table.Td>
 <Table.Td ta="right" style={{ }}>{fmt(est)}</Table.Td>
 <Table.Td ta="right" style={{color: pctUsed > 100 ? COLOR_ERROR_DARK : DEEP_BLUE, fontWeight: 600 }}>{fmt(act)}</Table.Td>
 <Table.Td ta="right">
 <Badge color={pctUsed > 100 ? 'red' : pctUsed > 95 ? 'orange' : 'yellow'} variant="light">
 {pctUsed.toFixed(1)}%
 </Badge>
 </Table.Td>
 <Table.Td ta="right" style={{color: remaining < 0 ? COLOR_ERROR_DARK : DEEP_BLUE }}>
 {fmt(remaining)}
 </Table.Td>
 </Table.Tr>
 );
