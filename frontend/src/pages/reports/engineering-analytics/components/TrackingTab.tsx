...
import {
 Stack, Group, Text, Badge, Select, Loader, Table, SegmentedControl, ScrollArea, Accordion, Pagination, Progress, Tooltip, Tabs, Anchor, Paper,
} from '@mantine/core';
...
import { jiraLink, fmt0, fmtDate } from '../utils';

export function TrackingTab({ projectKey }: { projectKey: string | null; avatars: Record<string, string> }) {
 ...
 queryKey: ['eng-analytics', 'tracking', 'risk-detection', projectKey],
 queryFn: async () => {
 ...
 queryKey: ['eng-analytics', 'tracking', 'seasonal-patterns', projectKey],
 queryFn: async () => {
 ...
 // Build set of epic keys that have orphaned open issues
 ...
 // Past   = released=true (officially shipped)
 // Future = not released AND (no date, OR date >= today, OR date < today — overdue but still unreleased)
 ...
 {/* Risk Detection — moved from Forecasting */}
 ...
 {/* Seasonal Patterns — moved from Forecasting */}
