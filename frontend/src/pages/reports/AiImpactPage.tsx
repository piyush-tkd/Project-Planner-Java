...
import {
 ...
 SegmentedControl, Center, Skeleton, Badge
} from '@mantine/core';
import {
...
import { AQUA_HEX, COLOR_BLUE_LIGHT, COLOR_ERROR, COLOR_ORANGE, DEEP_BLUE, GRAY_300, TEXT_DIM } from '../../brandTokens';
...
function KpiCard({ icon, label, value, sub, color }: KpiProps) {
 ...
 <div>
 <Text size="xs" c="dimmed" fw={500} style={{ }}>{label}</Text>
 <Text size="xl" fw={700} style={{ color: DEEP_BLUE, lineHeight: 1.2 }}>
 {value}
 </Text>
 {sub && <Text size="xs" c="dimmed" style={{ }}>{sub}</Text>}
 </div>
 </Group>
 </Paper>
 );
...
export default function AiImpactPage() {
 ...
 const { data: activeTrend, label: axisLabel } = trendMap[chartType];
 ...
 <div>
 <Title order={4} style={{ color: DEEP_BLUE}}>AI Impact Measurement</Title>
 ...
 {/* Chart selector */}
 <Group>
 <Text fw={600} size="sm" style={{ }}>Metric:</Text>
 <SegmentedControl
 size="xs"
 value={chartType}
