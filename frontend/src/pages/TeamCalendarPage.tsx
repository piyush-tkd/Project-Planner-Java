  ...
  return (
    <Stack gap="md" className="page-enter stagger-children">
      ...
      <Paper withBorder radius="md" style={{ overflow: 'hidden' }}>
        ...
        {/* ── Legend ─────────────────────────────────────────────────────── */}
        <Group gap="lg" px="md" py="sm" style={{ borderTop: '1px solid #e9ecef', background: '#f8f9fa' }} wrap="wrap">
          {LEGEND.map(l => (
...
function HeatmapCell({
  pct, style, projects, isCurrent, rowEven, tooltipTitle, tooltipExtra, onClick,
}: {
  pct: number;
  style: CellStyle;
  projects: ProjectPodMatrixResponse[];
  ...
  return (
    ...
    >
      <td
        ...
        style={{
          background: isEmpty ? (rowEven ? '#fff' : '#f8f9fa') : style.bg,
          color: style.text,
          textAlign: 'center',
          padding: '8px 4px',
          cursor: (!isEmpty && onClick) ? 'pointer' : 'default',
          borderLeft: isCurrent ? '2px solid #40c4c9' : '1px solid #e9ecef',
          borderBottom: '1px solid #e9ecef',
