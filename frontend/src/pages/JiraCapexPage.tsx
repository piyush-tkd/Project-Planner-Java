...
function IssueModal({
 ...
 title={
 <Group gap="xs">
 <Text fw={700} style={{ color: isDark ? '#fff' : DEEP_BLUE }}>{title}</Text>
...
function LocationCard({ loc }: { loc: LocationSummary }) {
 ...
 <Text size="xs" fw={600} style={{ color: isDark ? '#fff' : DEEP_BLUE }}>{fmtHours(loc.idsHours)}</Text>
 </Group>
 <Group justify="space-between">
...
export default function JiraCapexPage() {
 ...
 <Paper withBorder p="md" radius="md" mb="lg">
 <Group gap="xs" mb="md">
 <IconUsers size={16} color={isDark ? '#fff' : DEEP_BLUE} />
